_veh = _this select 0;

_veh addItemcargoGlobal ['ACE_morphine', 10];
_veh addItemcargoGlobal ['ACE_epinephrine', 10];
_veh addItemcargoGlobal ['ACE_fieldDressing', 30];
_veh addItemcargoGlobal ['ACE_tourniquet', 10];
_veh addItemcargoGlobal ['ACE_quikclot', 30];
_veh addItemcargoGlobal ['ACE_elasticBandage', 30];
_veh addItemcargoGlobal ['ACE_packingBandage', 30];
_veh addItemCargoGlobal ['ACE_splint', 10];

_veh additemcargoGlobal ['rhs_mag_30Rnd_556x45_M855A1_Stanag', 25];
_veh additemcargoGlobal ['rhs_mag_30Rnd_556x45_M855A1_Stanag_Tracer_Red', 10];
_veh additemcargoGlobal ['rhsusf_200Rnd_556x45_box', 10];
_veh additemcargoGlobal ['rhsusf_200rnd_556x45_mixed_box', 5];
_veh additemcargoGlobal ['mkk_20Rnd_762x51_m80a1_Mag', 15];
_veh additemcargoGlobal ['rhs_mag_M441_HE', 15];
_veh additemcargoGlobal ['rhs_mag_m67', 15];
_veh additemcargoGlobal ['rhs_mag_an_m8hc', 15];

_veh addBackpackCargoGlobal ['mkk_Bag_Felin_cec_desert_45L_Radio', 1];
_veh addBackpackCargoGlobal ['mkk_amf_felin_cec_des_backpack', 2];

_veh additemcargoGlobal ['LOP_H_6B27M_RACS', 2];
_veh addItemCargoGlobal ["ToolKit", 2];